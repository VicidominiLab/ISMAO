...
def transfer_cnnx1(path_weights,path_file,new_file,new_label,t_dset,t_labe):
    ...
    data = np.delete(new_file, 12, axis=3)[:,:,:,:,1]
    ...
    with tf.device('/CPU:0'):
        history = loaded_model.fit(data[0:s_e,:,:,:], new_label[0:s_e,:], epochs=30, batch_size=16, shuffle = True, validation_data = (data[s_e::,:,:,:], new_label[s_e::,:]))
   ...
   # print("Saved model to disk")
    ...
    dset = np.delete(t_dset[:,:,:,:,1], 12, axis=3)
